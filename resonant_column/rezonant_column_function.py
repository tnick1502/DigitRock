...
def define_G0_threshold_shear_strain(p_ref, E50, c, fi, K0, type_ground, Ip, e) -> tuple:
    ...
    dependence_Eur = {
        1: np.random.uniform(0.75, 0.8),  # Песок гравелистый
        2: np.random.uniform(0.8, 0.85),  # Песок крупный
        3: np.random.uniform(0.85, 0.9),  # Песок средней крупности
        4: np.random.uniform(0.85, 0.9),  # Песок мелкий
        5: np.random.uniform(0.9, 0.95),  # Песок пылеватый
        6: np.random.uniform(0.95, 1.05),  # Супесь
        7: np.random.uniform(1.05, 1.10),  # Суглинок
        8: np.random.uniform(1.15, 1.3),  # Глина
        9: np.random.uniform(0.75, 0.8),  # Торф
    }

    K_ground_type = E50 * dependence_Eur[type_ground]
