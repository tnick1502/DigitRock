...
def function_consalidation(final_volume_strain,
    ...
    max_time_log = np.log(max_time_sqrt ** 2 + 1)
    ...
    time_line_log = np.log(time_line_sqrt ** 2 + 1)

    # Расчет t_creep_sqrt, volume_strain_creep
    t_creep_log = t_90_log + abs(max_time_log - t_90_log)*np.random.uniform(0.4, 0.6)
    ...
    y_part5 = bezier_curve([t_90_log, volume_strain_90], [np.log(xK**2 + 1), yK], [t_creep_log, volume_strain_creep], [max_time_log, final_volume_strain],
...
if __name__ == "__main__":
    ...
    print(finale)
    # x1, y1, xp, yp = function_consalidation(finale, Cv=1,reverse=True, max_time=800, point_time=0.001, Ca=-0.1)
    x1, y1 = function_consalidation(-0.06113427426476852, Cv=0.1, reverse=True, max_time=500, point_time=1, Ca=-0.01082)
