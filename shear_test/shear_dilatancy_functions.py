...
def curve_shear_dilatancy(qf, e50, **kwargs):
    try:
        kwargs["xc"]
    except KeyError:
        kwargs["xc"] = 0.15
    ...
    index_05qf, = np.where(y >= 0.5 * qf)
    x_given = x_old[index_05qf[0]]
            ...
            for l in range(len(angle_of_dilatacy_all)):
                for m in range(len(angle_of_end_all)):
                    for n in range(len(v_d2_all)):
                        x, y, z = volumetric_deformation_test(m_given_all[i], v_d_xc_all[k],
...
def volumetric_deformation_test(m_given, v_d_xc, angle_of_dilatacy, angle_of_end, v_d2):
    x, y, z, a, b, c = curve_shear_dilatancy(30 * 5, 8220 * 5, xc=0.02, x2=0.15, qf2=100, m_given=m_given,
...
def dictionary_deviator_loading(strain, tau, vertical_strain, sigma, velocity):
    ...
    time_initial = np.hstack((np.full(5, 0.004), \
                   np.repeat(np.array([60 + np.random.uniform(0.3, 0.6),
    ...
    vertical_press = np.hstack((np.full(5, 0),
                                np.full(len(time) - 5,
                                        sigma + np.random.uniform(-1.5, 0.1))))
    ...
    stage = ['Пуск'] * 3 + ['Вертикальное нагружение'] * 3 + ['Срез'] * (len(time) - 6)
    ...
    data = {
        ...
        "SampleDiameter_mm": np.round(np.full(len(time), 71.4)),
        "VerticalPress_kPa": vertical_press,
        "VerticalDeformation_mm": vertical_deformation,
        "ShearDeformation_mm": shear_deformation,
        "ShearPress_kPa": shear_press,
        "Stage": stage
    }
