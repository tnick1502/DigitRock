...
class CyclicVibrationStrangth(ModelTriaxialDeviatorLoadingSoilTest):
    def set_test_params(self):
        self._test_params.frequency = 5
        self._test_params.sigma_d = 5
        self._test_params.step = 30
    ...
    def _test_modeling(self):
        ...
        split = CyclicVibrationStrangth.splitter(self._test_data.strain_cut, self._test_data.deviator_cut, self._test_params.step)

        offset = CyclicVibrationStrangth.strain_offset(strain_cut, deviator_cut, self._test_data.strain_cut, self._test_data.deviator_cut, self._test_params.step)
        ...
        for key in split:
            ...
            if (key != list(split.keys())[-1]):
                s, d = CyclicVibrationStrangth.cyclic_step(np.linspace(0, 500, 20 * 500 + 1), self._test_params.sigma_d,
                                                           offset[key]["offset"],
                                                           self._test_params.E50)
    ...
    @staticmethod
    def cyclic_step(cycles, deviator_amplitude, max_strain, E50):
    ...
    @staticmethod
    def splitter(strain_1: 'np.ndarray', deviator_1: 'np.ndarray', step: float = 20, eps=0.5):
        result = {}
...
class CyclicVibrationStrangthMohr(ModelMohrCirclesSoilTest):
    ...
    def _test_modeling(self):
        ...
        statment[statment.current_test].mechanical_properties.u = [np.round(np.random.uniform(0.3, 0.5) * (i / max(statment[statment.current_test].mechanical_properties.pressure_array["current"])), 1) for i in statment[statment.current_test].mechanical_properties.pressure_array["current"][:-1]]

        u = max(statment[statment.current_test].mechanical_properties.pressure_array["current"]) * np.random.uniform(0.3, 0.5)
        ...
        statment[statment.current_test].mechanical_properties.u = [np.round(u * np.random.uniform(0.8, 0.9) * (
         ...
         statment[statment.current_test].mechanical_properties.pressure_array["current"][:-1]] + [u]
