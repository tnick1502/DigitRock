...
def cyclic_path_processing(path):
    params = {}

    laboratory_numbers = folders_in_path(path)

    for laboratory_number in laboratory_numbers:
        params[laboratory_number] = find_test(os.path.join(path, laboratory_number), "Косинусное значение напряжения.txt")
